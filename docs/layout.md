...
Lily58 系より少しだけ記号に強い 60 キー構成です。左右それぞれ 30 キー、合計 60 キーにします。メイン部は基本 4 行 x 6 列に、T/Y の内側へ `-` / `+`、G/H の内側へ `[` / `]` を足します。親指クラスタは片手 4 キーにして、常用は押しやすい 3 キーに絞ります。
...
60 キーを維持しつつ、`-` と `+` は T/Y 内側に置く。

物理キーとしてさらに増やすなら 62 キー案にして、B/N の内側へ追加記号を置く。ただし B/N の内側は親指クラスタと近く、下段の人差し指移動も増えるため優先度を下げる。
